...
app = FastAPI( # 생성자를 통해서 postman을 대체하는 문서화 툴 내장되어 있음
    title="MBC AI 프로젝트 테스트",
    description = "파이썬과 자바투트를 연동한 ai 앱",
    version="1.0.0",
...
class LoggingMiddleware(BaseHTTPMiddleware): # 로그를 콘솔에 출력하는 용도
    ...
    async def dispatch(self, request: Request, call_next) :
        ...
        return response
app.add_middleware(LoggingMiddleware) # 모든 요청에 대해 로그를 남기는 middleware 클래스를 사요용함
...
@app.get("/items/{item_id}")    # http://아이피주소:포트/items/1
